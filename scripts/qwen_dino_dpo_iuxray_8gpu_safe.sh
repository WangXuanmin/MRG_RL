#!/bin/bash

dataset="iu_xray"
annotation="data/iu_xray/annotation.json"
base_dir="data/iu_xray/images"
preference_file="data/iu_xray/preferences_cap50_margin010_ref.jsonl"
sft_delta_file="save/iu_xray/qwen_dino_sft_8gpu_continue2/checkpoints/checkpoint_epoch1_step190_train.pth"
version="qwen_dino_dpo_safe_ref_cap50_margin010"
savepath="./save/${dataset}/${version}"

mkdir -p "${savepath}"

python -u train.py \
    --stage dpo \
    --dataset ${dataset} \
    --annotation ${annotation} \
    --base_dir ${base_dir} \
    --preference_file ${preference_file} \
    --delta_file ${sft_delta_file} \
    --vision_model resources/models/facebook_dinov2-base \
    --llm_model resources/models/Qwen3-8B \
    --llm_use_lora True \
    --llm_r 16 \
    --llm_alpha 32 \
    --resampler_num_queries 32 \
    --resampler_num_layers 2 \
    --dpo_objective reference \
    --dpo_require_ref_logps True \
    --dpo_sft_loss_weight 0.05 \
    --dpo_beta 0.05 \
    --batch_size 1 \
    --val_batch_size 4 \
    --max_epochs 1 \
    --max_length 160 \
    --learning_rate 1e-6 \
    --freeze_vm True \
    --savedmodel_path ${savepath} \
    --num_workers 8 \
    --devices 8 \
    --strategy ddp \
    --accelerator gpu \
    --precision bf16-mixed \
    --limit_val_batches 0 \
    --num_sanity_val_steps 0 \
    2>&1 | tee ${savepath}/log.txt
